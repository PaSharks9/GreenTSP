...
def salva_istanza(dizionario_Nearest_Neighbour, dizionario_Christofides, dizionario_Nearest_Neighbour_ott_ricarica, dizionario_citta, dizionario_stazioni, Max_Axis, k):
    ...
    name_NN_Ottimizzazione_Ricarica= "NearestNeighbour_Ottimizzazione_Ricarica" + str(totalDir + 1) + ".jpg"
    src_dir_NN_Ottimizzazione_Ricarica= "C:/Users/lucap/OneDrive/Desktop/GreenTSP/img/Nearest_Neighbour_ott_ricarica/NearestNeighbour_Ottimizzazione_Ricarica.jpg"
    ...
    dst_dir_NN_Ottimizzazione_Ric= dst_dir + name_NN_Ottimizzazione_Ricarica

    copyfile(src_dir_Christofides,dst_dir_Christofides)
    copyfile(src_dir_Nearest_Neighbour,dst_dir_Nearest_Neighbour)
    copyfile(src_dir_NN_Ottimizzazione_Ricarica,dst_dir_NN_Ottimizzazione_Ric)
    ...
    # Salvo il file della Nearest Neighbour con ottimizzazione della ricarica
    path_file= path_dir + '/'
    name_file= path_file + "Nearest_Neighbour_Ottimizzazione_Ricarica" + str(totalDir+1) + ".txt" 
    f= open(name_file, "w+")


    f.write("Percorso: " + str(dizionario_Nearest_Neighbour_ott_ricarica['percorso']))
    f.write("\nDistanza Percorsa: " + str(dizionario_Nearest_Neighbour_ott_ricarica['distanza']))
    f.write("\nTempo totale impiegato: "  + str(dizionario_Nearest_Neighbour_ott_ricarica['tempo_tot']))
    f.write("\nTempo ricarica: " + str(dizionario_Nearest_Neighbour_ott_ricarica['tempo_ricarica']))
...
def leggi_istanza():
    ...
    if scelta_directory != 0:
        name_file= APP_FOLDER + dir_name + '/dati_istanza_' + str(scelta_directory) + '.txt'
        f= open(name_file,"r")
